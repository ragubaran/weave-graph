...
fn ground_symbol(nodes: &[Node], name: &str) -> Result<NodeId, (String, Vec<String>)> {
    ...
    if let Some(node) = nodes
        .iter()
        .find(|n| n.symbol.to_lowercase() == name.to_lowercase())
    {
        return Ok(node.id);
    }
    ...
    let symbol = ground_symbol(nodes, &call.symbol)
        .map(|id| {
            nodes
                ...
                .find(|n| n.id == id)
                .map(|n| n.symbol.clone())
                .unwrap_or_else(|| call.symbol.clone())
        })
        .map_err(|(name, closest)| {
            if closest.is_empty() {
                ...
                format!(
                    "symbol not found in index: {name} (closest: {})",
                    closest.join(", ")
                )
            }
        })?;
    let second = match &call.second {
        Some(name) => Some(ground_symbol(nodes, name).map_err(|(name, closest)| {
            if closest.is_empty() {
                format!("symbol not found in index: {name}")
            } else {
                format!(
...
pub(crate) fn cmd_ask(
    ...
    let (routed, degraded) = match router.route(question, &symbols) {
        Ok(call) => (call, false),
        Err(RouterError::Unavailable(_) | RouterError::Malformed(_)) => (
            slm::FuzzyRouter
    ...
    finish(
        &storage, &grounded, &routed, route_ms, &model, degraded, dry_run, json,
    )
}
...
fn finish(
    ...
    let corrected = if grounded == routed {
        ""
    } else {
        " (corrected)"
    };
