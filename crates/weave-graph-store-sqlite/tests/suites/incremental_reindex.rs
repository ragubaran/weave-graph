...
fn assert_no_dangling_edges(s: &dyn Storage) {
...
/// (Core Invariant 3).
pub fn reindex_one_of_two_mutually_referencing_files_leaves_zero_dangling_edges(open: OpenFn) {
...
pub fn purge_file_edges_removes_edges_in_both_directions(open: OpenFn) {
    let (_dir, path) = tmp_db();
    let mut s = open(&path);
...
pub fn unrelated_file_edges_survive_targeted_purge(open: OpenFn) {
    let (_dir, path) = tmp_db();
    let mut s = open(&path);
...
// ─── Bulk bailout (storage-independent) ───────────────────────────────────────

pub fn bailout_threshold_triggers_on_large_change_fraction() {
...
pub fn bailout_floor_protects_tiny_repos() {
    let cfg = ReindexConfig::default();
