...
fn embed_produces_the_configured_dimension_count() {
    ...
    assert_eq!(provider.embed("hello world").unwrap().len(), 16);
...
fn embed_is_deterministic() {
    ...
    assert_eq!(
        provider.embed("check jwt ttl").unwrap(),
        provider.embed("check jwt ttl").unwrap()
    );
}
...
fn embed_is_l2_normalized() {
    let provider = MockEmbeddingProvider::new();
    let v = provider.embed("check jwt ttl verify token").unwrap();
...
fn shared_vocabulary_is_closer_than_disjoint_vocabulary() {
    let provider = MockEmbeddingProvider::new();
    let a = provider.embed("check jwt token expiry").unwrap();
    let b = provider.embed("verify jwt token lifetime").unwrap();
    let c = provider.embed("render html template layout").unwrap();
