...
fn walk(node: Node, source: &[u8], path: &str, scope: &[String], file: &mut ParsedFile) {
    ...
    for child in node.children(&mut cursor) {
        match child.kind() {
            "trait_definition" | "class_definition" => {
                let Some(name) = child.child_by_field_name("name") else {
                    continue;
                };
                let name = text(name, source);
                let kind = if child.kind() == "trait_definition" {
                ...
                if let Some(extend) = child.child_by_field_name("extend")
                    && let Some(target) = extend.child_by_field_name("type")
                {
            ...
            "import_declaration" => {
                ...
                if let Some(leaf) = child
                    .children_by_field_name("path", &mut c)
