...
| --- | --- | --- |
| PERF-G01 | Partial | The existing 500k SQLite+CSR CI gate passes locally at 18 MiB peak RSS (500,000 nodes/499,999 edges, 80 MiB budget). `scripts/pipeline_rss.sh` now generates a deterministic 500k-symbol corpus and meters the actual `weave index` process; CI invokes it, but retained Linux results are still required before closure. |
