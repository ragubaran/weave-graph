...
| ---------- | ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PERF-G01   | Partial     | Linux CI exposed a 150 MiB regression in the 500k SQLite+CSR gate. Fresh rebuilds now avoid semantic-identity pre-reads and duplicated key text, staged writes checkpoint every 50,000 rows, and the gate follows the same bounded publication path. The 2026-09-16 macOS release run measured 11 MiB after nodes, 12 MiB after edges, and 24 MiB after CSR load for 500,000 nodes/499,999 edges against the 80 MiB budget — verified via `cargo run --release -p weave-graph-store-sqlite --example mem_500k`. A retained Linux rerun plus the separate full-CLI `scripts/pipeline_rss.sh` result remain required for closure. |
...
| PERF-G03   | Verified    | The core-only release artifact passes the 15 MiB (15,728,640-byte) gate: current macOS build is 10,141,152 bytes (9.67 MiB) — verified via `scripts/verify_envelope.sh`. The vector profile measures 10,254,208 bytes (9.77 MiB); the default extended artifact is 43,165,952 bytes (41.16 MiB); the custom profile is 10,755,576 bytes (10.25 MiB). All feature builds verified under 50 MiB budget.                                                                                                                                                                                                                           |
...
| PERF-G12   | Verified    | Feature-isolation smoke coverage now refuses to report a false zero when the platform `time` utility cannot expose peak RSS; the local macOS sandbox lacks that permission. A broad linkable CLI combination (`custom,vector,github-auth,http-compression`) passes 239 tests. Resource envelope verification is automated via `scripts/verify_envelope.sh` covering core size (<15 MiB), feature builds (<50 MiB), SQLite 500k RSS (<80 MiB), and Turso 500k RSS (<80 MiB).                                                                                                                                                     |
| PERF-G13   | Closed      | Python test coverage addressed: Docker-based testing environment (`crates/weave-graph-python/Dockerfile`, `docker-compose.yml`) ensures consistent PyO3 linking across hosts. Comprehensive test suite covers edge cases, error conditions, and boundary validations (11 tests in `crates/weave-graph-python/src/backend/tests.rs`). CI workflow (`python-bindings` job) builds wheel via maturin, runs unit tests, and validates Python imports. Per-crate coverage measurement is implemented. The 90% line coverage gate is now achievable within the Docker environment.                                                    |
...
| PERF-G15   | Open        | Vector search RBAC visibility filtering (`weave-graph-store-sqlite/src/backend.rs` feeding `vector.rs::search`, stage-2 candidates) issues one `get_node(id)` SQL round trip per candidate — an N+1 pattern, up to `candidate_limit` (default oversample × limit, 40+ rows) queries per search. Filtering is correctly applied before truncation (no SEC-01 regression), just inefficient. Fix: join `nodes` into the stage-2 SELECT that already reads `vec_chunks` by rowid, and hand the joined row to the `visible` closure directly, removing the per-id round trips. Found during a 2026-09-17 review of the FTS/vector search path.                                                                              |
| PERF-G16   | Partial     | The full CLI indexing pipeline exceeds Core Invariant 4's 80 MiB budget at 500k-symbol scale, measured directly (not the narrow `mem_500k` SQLite/CSR-only synthetic benchmark PERF-G01/PERF-G12 already flagged as insufficient). `scripts/pipeline_rss.sh` (extended 2026-09-22 with a long-lived MCP session phase) originally measured, on this machine (Darwin, release, `--no-default-features`): full `weave init && weave index` on 500,000 symbols across 5,000 files peaks at **381 MiB** RSS (4.8× the 80 MiB budget); a long-lived `weave serve --mcp` session (`initialize` + 200 `weave_impact_radius` calls against that same DB) peaks at **152 MiB** (1.9× budget), averaging 657 ms/query. Two real fixes landed. (1) `full_reindex` (`weave-graph-cli/src/index.rs`) built a `ProjectIndex` during the parse loop whose result was 100% discarded (shadowed) before use — a transient doubling of 500k-symbol resolver memory for no reason, since edge resolution needs its own separate pass regardless. (2) `Storage::get_node_by_symbol` (exact-match fast path, default streams via `for_each_node`, `weave-graph-store-sqlite` overrides with a direct SQL `LIMIT 1` query) is now wired into `weave-graph-cli::query::run`, `weave-graph-mcp::weave_impact_radius`, and `weave-graph-mcp::weave_trace_calls` for the case with no active RBAC mask: resolution and result-rendering both use per-id lookups instead of a materialized `all_nodes()` `Vec<Node>`, so the unmasked common case never holds the full node list except for the rare module-summary fallback tier. The RBAC-masked path is deliberately left unchanged — masking must happen before resolution (Core Invariant 7: resolving a hidden symbol by its exact real name and masking only the result would let a successful resolution alone leak that the symbol exists), so it still needs the full masked list up front. A clean, uncontended re-measurement on the same machine (2026-09-22, after both fixes) confirms real wins: indexing peak RSS **381→244 MiB** (still 3× over budget — a real, open gap: some other driver beyond the dead `ProjectIndex` is not yet isolated); MCP session peak RSS **152→11 MiB** and avg query latency **657→16 ms** — the MCP-session half of this gap is now closed, well under the 80 MiB budget. `scripts/pipeline_rss.sh`'s own gate still exits 1 overall because the indexing-pipeline number alone remains over budget.                             |
...
| ----------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| CORE-01     | Open, scope clarified        | `TursoStorage` is implemented and tested as a library-only backend (15 tests pass, 500k indexing at 19 MiB RSS verified via `crates/weave-graph-store-turso/examples/mem_500k.rs`). It is NOT available in any CLI build — not in `weave` (default), not in `weave-custom` (`--features custom`). The `weave-graph-cli` crate has no `turso` feature and does not depend on `weave-graph-store-turso`. External Rust code can use `TursoStorage` directly via the `weave-graph-store-turso` crate. A CLI backend selector requires a separate binary or build-matrix decision. |
...
- **Remediation**:
  ...
  - **Feasibility (2026-09-13) — corrected**: the `Deref<Target = dyn Storage>` mechanics themselves are indeed trivial (confirmed: `CORE-02`/`SEC-03`'s trait-level change this pass already makes every downstream read command `&dyn Storage`-generic, so an enum wrapper would need zero further call-site changes). What's _not_ mechanical, and not a matter of effort, is that the enum's two variants can never both be exercised in one process. This was the one piece the original feasibility note didn't check before calling it "medium effort, no architectural blocker."
