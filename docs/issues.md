...
| AUTH-GH-01 | Partial | Optional `github-auth` resolves `WEAVE_GITHUB_TOKEN` through GitHub `/user`, applies configured `[rbac.github_roles]`, and now has a local HTTP mock proving bearer transmission and response parsing. Retry behavior and organization/team lookup remain open. |
