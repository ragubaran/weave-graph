...
| --- | --- | --- |
| PERF-G01 | Partial | Linux CI exposed a 150 MiB regression in the 500k SQLite+CSR gate. Fresh rebuilds now avoid semantic-identity pre-reads and duplicated key text, staged writes checkpoint every 50,000 rows, and the gate follows the same bounded publication path. The 2026-09-16 macOS release run measured 23 MiB after nodes, 24 MiB after edges, and 31 MiB after CSR load for 500,000 nodes/499,999 edges against the 80 MiB budget. A retained Linux rerun plus the separate full-CLI `scripts/pipeline_rss.sh` result remain required for closure. |
